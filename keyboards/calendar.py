...
class SimpleCalendar:
    ...
    async def start_calendar(
            ...
            banned_days: Optional[list] = []
    ) -> InlineKeyboardMarkup:
        """
        Creates an inline keyboard with the provided year and month
        :param int year: Year to use in the calendar, if None the current year is used.
        :param int month: Month to use in the calendar, if None the current month is used.
        :param int banned_days: Month to use in the calendar, if None the current month is used.
        ...
        for idx, day in enumerate(days):
            ...
            for item in month_calendar:
                res[day].append(item[idx])
        for week in month_calendar:
            inline_kb.row()
            # print(month, datetime.now().month)
            for day in week:
                if (day == 0) or (day in banned_days) or day in res["Сб"] or day in res["Вс"]:
